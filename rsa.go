...
func RightUnPad(d []byte) ([]byte, error) {
	var i = len(d) - 1
	for ; i >= 0; i-- {
		if d[i] != 0 {
			break
...
// GetPubKey 读取公钥
func GetPubKey(in string) (*rsa.PublicKey, error) {
	key, err := base64.StdEncoding.DecodeString(in)
	if err != nil {
		return nil, err
	}

	return GetPubKeyFromPem(key)
}

// GetPubKeyFromPem 从PEM中读取公钥
func GetPubKeyFromPem(key []byte) (*rsa.PublicKey, error) {
...
// GetPriKey 读取私钥
func GetPriKey(in string) (*rsa.PrivateKey, error) {
	key, err := base64.StdEncoding.DecodeString(in)
	if err != nil {
		return nil, err
	}
...
// bigZero, bigOne  copy from crypto/rsa
var bigZero = big.NewInt(0)
var bigOne = big.NewInt(1)

// encrypt copy from crypto/rsa
...
// copyWithLeftPad copy from crypto/rsa
...
// nonZeroRandomBytes copy from crypto/rsa
...
// modInverse copy from crypto/rsa
